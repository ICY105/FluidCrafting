...
execute align xyz positioned ~ ~1 ~ as @e[dx=0,dy=0,dz=0] at @s run function fluid:v1.5/pipe/break_2
...
execute as @e[type=#fluid:valid_tank_entities,tag=fluid.tank,predicate=fluid:v1.5/has_network_id] at @s run function fluid:v1.5/pipe/break_reconnect

#setup network transfer capacity
scoreboard players set @e[tag=fluid.pipe] fluid.transfer_capacity -1
execute as @e[tag=fluid.pipe] at @s run function fluid:v1.5/pipe/init_network_transfer_rate
